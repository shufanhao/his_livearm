...
inline HI_U16 OSD_MAKECOLOR_U16(HI_U8 r, HI_U8 g, HI_U8 b,
	...
	pixel |= (r1 | (g1 << compinfo.blen)
			| (b1 << (compinfo.blen + compinfo.glen)));
	return pixel;
}
			...
			if (Bpp == 3)/*.....*/
			{
				...
				case OSD_COLOR_FMT_RGB8888:
					memcpy(pRGBBuf + i * pVideoLogo->stride + j * 4,
							pOrigBMPBuf + ((h - 1) - i) * stride + j * Bpp,
							Bpp);
...
HI_S32 LoadBitMap2Surface(const HI_CHAR *pszFileName,
		const OSD_SURFACE_S *pstSurface, HI_U8 *pu8Virt) {
	OSD_LOGO_T stLogo;
...
HI_S32 CreateSurfaceByBitMap(const HI_CHAR *pszFileName,
		OSD_SURFACE_S *pstSurface, HI_U8 *pu8Virt) {
	OSD_LOGO_T stLogo;
	stLogo.pRGBBuffer = pu8Virt;
