...
HI_S32 SAMPLE_COMM_VENC_GetFilePostfix(PAYLOAD_TYPE_E enPayload,
	...
	if (PT_H264 == enPayload) {
		strcpy(szFilePostfix, ".h264"); //将.h264放到szFilePostfix.h264
...
HI_S32 SAMPLE_COMM_VENC_SaveH264(FILE* fpH264File, VENC_STREAM_S *pstStream) {
	...
	for (i = 0; i < pstStream->u32PackCount; i++) {
		...
		fwrite(buffer,size,count,fp)
		 ...
		 count表示要读写的数据块的块数
		 fp表示文件指针
		 调用fwrite函数之后，文件是被写到缓冲区的，用fflush函数之后，
		 会把缓冲区中的文件写到文件系统中
		printf("在函数SaveH264中pstStream->u32PackCount= %d\n",pstStream->u32PackCount);
		 printf("pstStream->pstPack[0].pu8Addr= %d\n",pstStream->pstPack[i].pu8Addr[0]);
		//
		...
		if (pstStream->pstPack[i].u32Len[1] > 0) {
			//printf("pstStream->pstPack[1].pu8Addr= %d",pstStream->pstPack[i].pu8Addr[2]);
...
HI_S32 SAMPLE_COMM_VENC_Start(VENC_GRP VencGrp, VENC_CHN VencChn,
	...
	case PT_H264: {
		...
		if (SAMPLE_RC_CBR == enRcMode) {
			...
			stH264Cbr.fr32TargetFrmRate =
					(VIDEO_ENCODING_MODE_PAL == enNorm) ? 25 : 20;/* target frame rate 这个地方可以修改目标帧率，就是让输出帧率变大或小*/
			switch (enSize) {
			case PIC_QCIF:
			...
			case PIC_CIF:
				stH264Cbr.u32BitRate = 512; //这个地方应该就是码率，越高则精度越大
				break;
			case PIC_D1:
			case PIC_VGA: /* 640 * 480 */
				stH264Cbr.u32BitRate = 768;
				//stH264Cbr.u32BitRate = 300;
				break;
			case PIC_HD720: /* 1280 * 720 */
				stH264Cbr.u32BitRate = 1024 * 2; //这个地方是不是输出的码率呢。
				//stH264Cbr.u32BitRate = 1024;				 //是不是可以修改数据波特率呢。
				break;
			...
			stH264Cbr.u32FluctuateLevel = 0; /* average bit rate 波动等级 */
			...
			stH264Vbr.u32Gop = (VIDEO_ENCODING_MODE_PAL == enNorm) ? 2 : 3;
	...
	case PT_MJPEG: {
		...
		memcpy(&stVencChnAttr.stVeAttr.stAttrMjpeg, &stMjpegAttr,
				sizeof(VENC_ATTR_MJPEG_S));

		if (SAMPLE_RC_FIXQP == enRcMode) {
			stVencChnAttr.stRcAttr.enRcMode = VENC_RC_MODE_MJPEGFIXQP;
			stMjpegeFixQp.u32Qfactor = 90;
			stMjpegeFixQp.u32ViFrmRate =
					(VIDEO_ENCODING_MODE_PAL == enNorm) ? 25 : 30;
			stMjpegeFixQp.fr32TargetFrmRate =
					(VIDEO_ENCODING_MODE_PAL == enNorm) ? 25 : 30;
			...
			stVencChnAttr.stRcAttr.stAttrMjpegeCbr.u32ViFrmRate =
					(VIDEO_ENCODING_MODE_PAL == enNorm) ? 25 : 30;
			stVencChnAttr.stRcAttr.stAttrMjpegeCbr.fr32TargetFrmRate =
					(VIDEO_ENCODING_MODE_PAL == enNorm) ? 25 : 30;
			...
			case PIC_HD720: /* 1280 * 720 */
				...
				break;
			case PIC_HD1080: /* 1920 * 1080 */
				stVencChnAttr.stRcAttr.stAttrMjpegeCbr.u32BitRate = 1024 * 10
						* 3;
				break;
			default:
				stVencChnAttr.stRcAttr.stAttrMjpegeCbr.u32BitRate = 1024 * 7
						* 3;
				break;
			}
		} else if (SAMPLE_RC_VBR == enRcMode) {
			...
			stVencChnAttr.stRcAttr.stAttrMjpegeVbr.u32ViFrmRate =
					(VIDEO_ENCODING_MODE_PAL == enNorm) ? 25 : 30;
			...
			case PIC_HD720: /* 1280 * 720 */
				...
				break;
			case PIC_HD1080: /* 1920 * 1080 */
				stVencChnAttr.stRcAttr.stAttrMjpegeVbr.u32MaxBitRate = 1024 * 6
						* 3;
				break;
			default:
				stVencChnAttr.stRcAttr.stAttrMjpegeVbr.u32MaxBitRate = 1024 * 4
						* 3;
				break;
			}
		} else {
	 ...
	 ******************************************/
	s32Ret = HI_MPI_VENC_StartRecvPic(VencChn); //开启编码通道接受输入图像
	if (HI_SUCCESS != s32Ret) {
...
HI_VOID SAMPLE_COMM_VENC_GetVencStreamProc(HI_VOID *p) {
	...
	SAMPLE_VENC_GETSTREAM_PARA_S *pstPara; //这是一个线程指针
	HI_S32 maxfd = 0;
	struct timeval TimeoutVal;
	fd_set read_fds;
	...
	//s32ChnTotal = pstPara->s32Cnt;//这个值是3
	s32ChnTotal = 3;
	/******************************************
	...
	for (i = 0; i < s32ChnTotal; i++) {
		...
		enPayLoadType[i] = stVencChnAttr.stVeAttr.enType;    //将获得的编码通道属性的分辨率传给
		//根据编码方式获得文件的后缀
		...
		//sprintf的第一个参数应该是目的字符串
		sprintf(aszFileName[i], "stream_chn%d%s", i, szFilePostfix);
		//要打开文件
		pFile[i] = fopen(aszFileName[i], "wb");     // wb 只写打开或新建一个二进制文件；只允许写数据。
		if (!pFile[i]) {
		...
		/* Set Venc Fd. */
		VencFd[i] = HI_MPI_VENC_GetFd(i);    //将编码通道映射为一个文件句柄，并且调用此函数来获得该通道的文件句柄
											 //此函数返回值VencFd[i]即是文件句柄
		if (VencFd[i] < 0) {
	...
	/******************************************
	 ...
	 在这里面就要不断的循环了。也就是说在这个线程里面是要不断的运行这个函数然后
	 并且一段getchar()两个回车之后就停止运行程序
	 要修改这里面的dong xi
	 ******************************************/
	while (HI_TRUE == pstPara->bThreadStart) {
		...
		TimeoutVal.tv_sec = 2;
		TimeoutVal.tv_usec = 0;
		//select 函数用法http://blog.csdn.net/ccjjyy/article/details/6004422
		//此函数返回值即是返回处于就绪状态并且已经包含在fd_set结构中的描述字总数；如果超时则返回0；
		...
		if (s32Ret < 0) {
			...
			for (i = 0; i < s32ChnTotal; i++) {
				if (FD_ISSET(VencFd[i], &read_fds)) //判断如果Venc[i]是集合read_fds中的一员，非0，否则为0
						{
					/*******************************************************
					 step 2.1 : query how many packs in one-frame stream.
					 *******************************************************/
					memset(&stStream, 0, sizeof(stStream));        //先赋值0
...
HI_S32 SAMPLE_COMM_VENC_StartGetStream(HI_S32 s32Cnt) {
	/*gs_stPara.bThreadStart = HI_TRUE;
	 gs_stPara.s32Cnt = s32Cnt;//这个s32Cnt的值是3,也就说是三个通道
	 //第一个参数为指向线程标识符的指针，程序的ID； 第二个参数用来设置线程属性
	 //第三个参数是线程运行函数的起始地址。最后一个参数是线程运行函数的参数
	 //创建线程成功后返回值是0
