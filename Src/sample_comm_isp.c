...
#include <hi_include/sample_comm.h>//<hi_include/sample_comm.h>//#include "sample_comm_isp.h"static pthread_t gs_IspPid;
...
HI_S32 SAMPLE_COMM_ISP_Reg2File(HI_CHAR* pcCfgFile) {
	...
	while (s32FileLen < filestat.st_size) {
		...
		if (curbuf[0] == '#') {
			...
			snprintf(RegFmt, sizeof(RegFmt), "0x%08x:0x%08x", u32RegAddr,
					u32RegVal);
...
HI_S32 SAMPLE_COMM_ISP_Init(void) {
	...
	switch (APTINA_AR0130_DC_720P_30FPS) {
...
#if 1
	if (0 != pthread_create(&gs_IspPid, 0, (void* (*)(void*)) HI_MPI_ISP_Run,
	NULL)) {
