...
HI_S32 SAMPLE_COMM_VDA_MdPrtObj(FILE *fp, VDA_DATA_S *pstVdaData) {
	...
	for (i = 0; i < pstVdaData->unData.stMdData.stObjData.u32ObjNum; i++) {
		...
		fprintf(fp, "[%d]\t left=%d, top=%d, right=%d, bottom=%d\n", i,
				pstVdaObj->u16Left, pstVdaObj->u16Top, pstVdaObj->u16Right,
				pstVdaObj->u16Bottom);
	}
	fflush(fp);
	return HI_SUCCESS;
...
HI_S32 SAMPLE_COMM_VDA_MdPrtSad(FILE *fp, VDA_DATA_S *pstVdaData) {
	...
	for (i = 0; i < pstVdaData->u32MbHeight; i++) {
		pAddr =
				(HI_VOID *) ((HI_U32) pstVdaData->unData.stMdData.stMbSadData.pAddr
		...
		for (j = 0; j < pstVdaData->u32MbWidth; j++) {
			...
			if (VDA_MB_SAD_8BIT
					== pstVdaData->unData.stMdData.stMbSadData.enMbSadBits) {
...
HI_S32 SAMPLE_COMM_VDA_OdPrt(FILE *fp, VDA_DATA_S *pstVdaData) {
	...
	for (i = 0; i < pstVdaData->unData.stOdData.u32RgnNum; i++) {
		fprintf(fp, "OD region[%d]: %d\n", i,
				pstVdaData->unData.stOdData.abRgnAlarm[i]);
	}
	fflush(fp);
	return HI_SUCCESS;
}
...
HI_S32 SAMPLE_COMM_VDA_MdStart(VDA_CHN VdaChn, VI_CHN ViChn, SIZE_S *pstSize) {
	...
	if (VDA_MAX_WIDTH < pstSize->u32Width
			|| VDA_MAX_HEIGHT < pstSize->u32Height) {
	...
	pthread_create(&gs_VdaPid[SAMPLE_VDA_MD_CHN], 0,
			SAMPLE_COMM_VDA_MdGetResult, (HI_VOID *) &gs_stMdParam);
...
HI_S32 SAMPLE_COMM_VDA_OdStart(VDA_CHN VdaChn, VI_CHN ViChn, SIZE_S *pstSize) {
	...
	stVdaChnAttr.unAttr.stOdAttr.astOdRgnAttr[0].stRect.u32Width =
			pstSize->u32Width;
	stVdaChnAttr.unAttr.stOdAttr.astOdRgnAttr[0].stRect.u32Height =
			pstSize->u32Height;
	...
	pthread_create(&gs_VdaPid[SAMPLE_VDA_OD_CHN], 0,
			SAMPLE_COMM_VDA_OdGetResult, (HI_VOID *) &gs_stOdParam);
