...
HI_S32 SAMPLE_COMM_VO_StartDevLayer(VO_DEV VoDev, VO_PUB_ATTR_S *pstPubAttr,
	...
	s32Ret = SAMPLE_COMM_VO_GetWH(pstPubAttr->enIntfSync, &u32Width, &u32Height,
			&u32Frm);
	if (s32Ret != HI_SUCCESS) {
		SAMPLE_PRT("failed with %#x!\n", s32Ret);
		return HI_FAILURE;
...
HI_S32 SAMPLE_COMM_VO_StartChn(VO_DEV VoDev, VO_PUB_ATTR_S *pstPubAttr,
	...
	for (i = 0; i < u32WndNum; i++) {
		stChnAttr.stRect.s32X = ALIGN_BACK(
				(u32Width / u32Square) * (i % u32Square), 2);
		stChnAttr.stRect.s32Y = ALIGN_BACK(
				(u32Height / u32Square) * (i / u32Square), 2);
